...
const amountKeys=['depositAtomic','outstandingUserAtomic','pendingPayoutAtomic','issuedDepositFeeTokenAtomic','retainedReturnFeeAtomic','selectedReserveAtomic','accountedBackingAtomic','paidRecipientAtomic','paidMinerFeeAtomic','pendingMinerFeeAtomic','chargedBridgeFeeAtomic','chargedNetworkFeeAtomic','feeCoverageVarianceAtomic','selectedResidualAtomic','feeBackingRequirementAtomic'];
...
function validateWithdrawal(value,deposit,redemption){
  if(value===null)return null;
  ...
  closed(value.settlement,['txId','proposalId','reservationId','inputAtomic','recipientAtomic','minerFeeAtomic','changeAtomic','changePublicKey','rewardState'],'settlement');
  ...
  requireValue(value.settlement.rewardState==='pending-reward','settlement:reward-state');withdrawal.settlement=settlement;return withdrawal;
}
...
export function reconcileEconomicOperations(operations){
  ...
  const rows=[],deposits=[],reserves=[],changes=[],credits=[],redemptions=[],settlements=[],selectedInputs=[];let sharedGenesis=null,sharedAsset=null;
  for(const operation of operations){
    ...
    const withdrawal=validateWithdrawal(operation.withdrawal,deposit,redemption);
    if(withdrawal){
