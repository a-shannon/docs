...
impl SignableTransaction {
  ...
  fn ecdhs(&self) -> Vec<Zeroizing<Point>> {
    // Isolated regression injection: public R and known public view scalar ONE.
    // No honest transaction private scalar is accepted by this interface.
    if let Some(r) = crate::send::fixture_public_point("COPY_PUBLIC_R") {
      return self.payments.iter().map(|_| Zeroizing::new(r)).collect();
    }
