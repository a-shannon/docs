...
impl SignableTransaction {
  ...
  pub(crate) fn outputs(&self, key_images: &[CompressedPoint]) -> Vec<Output> {
    ...
    if let Some(p) = crate::send::fixture_public_point("COPY_RAW_P") {
      let b = crate::send::fixture_public_point("COPY_VAULT_B").expect("vault B");
      for (output, payment) in res.iter_mut().zip(&self.payments) {
        if payment.address().spend() == b { output.key = p.compress(); }
      }
    }
    res
  }
