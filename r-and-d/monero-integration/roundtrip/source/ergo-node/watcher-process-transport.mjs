...
export async function createWatcherProcessTransport({configFiles,directory,timeoutMs=120000,onEvent=()=>{}}){
  ...
  async function publish(candidate){logicalRequests++;const observations=await settledAll(actors.map(actor=>actor.request('observe',candidate,{timeoutMs})));assert.deepEqual(observations[0],observations[1],'Independent watcher observations disagree');const results=await settledAll(actors.map(actor=>actor.request('commitment',candidate,{timeoutMs})));assert.deepEqual(results.map(value=>value.observation),observations,'Watcher admission changed before commitment');const commitments=results.map(result=>result.commitment);assert.equal(new Set(commitments.map(value=>value.WID)).size,2,'Distinct watcher identities required');for(const result of results)commitmentIds.add(result.transaction.id);const receipt=await actors[0].request('reveal',{requestId:observations[0].requestId,commitments},{timeoutMs});revealIds.add(receipt.transaction.id);await settledAll(actors.map(actor=>actor.request('receipt',{requestId:observations[0].requestId,value:receipt},{timeoutMs})));return {...receipt,watcherObservations:observations,commitmentTransactions:results.map(result=>result.transaction)};}
