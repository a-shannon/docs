...
export interface VerifiedEventOrderSnapshot {
  readonly event: Readonly<EventTrigger>;
  readonly triggerTransactionId: string;
  readonly triggerBoxId: string;
  readonly wids: readonly string[];
  readonly feeConfig: Readonly<ChainMinimumFee>;
}

const logger = DefaultLogger.getInstance().child(import.meta.url);

class TransactionVerifier {
  /** Capture exactly the values consumed by the real payment-order comparison. */
  static verifyMoneroEventOrderSnapshot = async (
    tx: PaymentTransaction,
    observedEvent: EventTrigger,
    triggerTransactionId: string,
    triggerBoxId: string,
  ): Promise<Readonly<VerifiedEventOrderSnapshot> | undefined> => {
    if (tx.network !== 'monero' || tx.txType !== TransactionType.payment) return;
    const event = Object.freeze({ ...observedEvent });
    const originalFee = MinimumFeeHandler.getEventFeeConfig(event);
    const feeConfig = Object.freeze({ bridgeFee: originalFee.bridgeFee, networkFee: originalFee.networkFee, rsnRatio: originalFee.rsnRatio, rsnRatioDivisor: originalFee.rsnRatioDivisor, feeRatio: originalFee.feeRatio, feeRatioDivisor: originalFee.feeRatioDivisor });
    const txOrder = structuredClone(ChainHandler.getInstance().getChain(tx.network).extractTransactionOrder(tx));
    const wids = Object.freeze((await DatabaseAction.getInstance().getEventCommitments(tx.eventId)).map(commitment => commitment.WID));
    const expectedOrder = await EventOrder.createEventPaymentOrder(event, triggerTransactionId, feeConfig, [...wids]);
    if (!isEqual(txOrder, expectedOrder)) return;
    return Object.freeze({ event, triggerTransactionId, triggerBoxId, wids, feeConfig });
  };
  /**
