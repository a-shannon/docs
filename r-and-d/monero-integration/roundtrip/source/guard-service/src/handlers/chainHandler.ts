...
import type { MoneroPaymentLifecycle } from '../transaction/moneroPaymentLifecycle';
...
class ChainHandler {
  ...
  private scopedChains?: ReadonlyMap<string, AbstractChain<unknown>>;
  private moneroLifecycle?: MoneroPaymentLifecycle;
  ...
  private constructor(scoped?: { chains: ReadonlyMap<string, AbstractChain<unknown>>; moneroLifecycle: MoneroPaymentLifecycle }) {
    if (scoped) {
      this.scopedChains = new Map(scoped.chains);
      this.moneroLifecycle = scoped.moneroLifecycle;
      if (!this.scopedChains.has('monero')) throw Error('Scoped Monero chain missing');
      return;
    }
    AddressManager.init(
      chainValidators,
  ...
  /** Explicit local bootstrap: constructs only the supplied chain adapters. */
  public static initializeScoped = (chains: ReadonlyMap<string, AbstractChain<unknown>>, moneroLifecycle: MoneroPaymentLifecycle) => {
    ChainHandler.instance = new ChainHandler({ chains, moneroLifecycle });
    return ChainHandler.instance;
  };

  getMoneroPaymentLifecycle = (): MoneroPaymentLifecycle => {
    if (!this.moneroLifecycle) throw Error('Monero payment lifecycle is not registered');
    return this.moneroLifecycle;
  };
  ...
  getChain = (chain: string): AbstractChain<any> => {
    if (this.scopedChains) {
      const supplied = this.scopedChains.get(chain);
      if (!supplied) throw Error(`Scoped chain [${chain}] is not registered`);
      return supplied;
    }
    switch (chain) {
      case ERGO_CHAIN:
